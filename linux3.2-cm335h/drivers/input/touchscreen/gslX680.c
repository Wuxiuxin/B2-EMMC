...
static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
	...
	if(is_linux > 0)
	{
		...
		if(pressure != 0)
		{
			input_report_abs(ts->input, ABS_PRESSURE,200);
...
//	input_mt_init_slots(input_device, (MAX_CONTACTS + 1));
	...
	ret =  gpio_request(46,"int_gslX680");
	if(ret < 0)
	{
		printk("error to request int_gslX680\r\n");
		return -1;
	}
	client->irq = gpio_to_irq(46);
